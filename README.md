...
- Runtime repair rehydrates configured non-native providers from `/etc/openclaw-codeshield/models.d/*.conf` back into the protected `openclaw.json` files, preventing Telegram `/model` switches from silently falling back after updates or guardian syncs.
...
- it rehydrates configured non-native provider runtime blocks from `/etc/openclaw-codeshield/models.d/*.conf`, so `models.providers.deepseek` and similar entries survive repairs and guardian syncs
