...
seed_service_auth_state_once() {
    local rel existing=0
    for rel in \
        "agents/main/agent/auth.json" \
        "agents/main/agent/auth-profiles.json" \
        "identity/device-auth.json"; do
        if [ -f "$OPENCLAW_SVC_HOME/.openclaw/$rel" ]; then
            existing=1
            break
        fi
    done
    ...
    for rel in \
        ...
        "agents/main/agent/auth-profiles.json" \
        "identity/device-auth.json"; do
        local src="$OPENCLAW_HOME/.openclaw/$rel"
        local dst="$OPENCLAW_SVC_HOME/.openclaw/$rel"
        if [ -f "$src" ] && [ ! -f "$dst" ]; then
            mkdir -p "$(dirname "$dst")"
            install -m 0600 -o openclaw-svc -g openclaw-svc "$src" "$dst"
        fi
    done
}
