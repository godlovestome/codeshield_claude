...
MODELS_DIR="$CS_CONF_DIR/models.d"
...
sync_configured_model_providers() {
    python3 - "$MODELS_DIR" "/run/openclaw-codeshield/secrets.env" "$SECRETS_FILE" "$OPENCLAW_HOME/.openclaw/openclaw.json" "$OPENCLAW_SVC_HOME/.openclaw/openclaw.json" <<'PY'
import json
import sys
from pathlib import Path

models_dir = Path(sys.argv[1])
secret_files = [Path(sys.argv[2]), Path(sys.argv[3])]
json_files = [Path(p) for p in sys.argv[4:]]
...
provider_specs = {
    ...
    'glm5': {
        ...
        'api': 'openai-completions',
        'models': [
            {'id': 'glm-4-flash', 'name': 'GLM-4-Flash', 'reasoning': False, 'input': ['text']},
            {'id': 'glm-4-air', 'name': 'GLM-4-Air', 'reasoning': False, 'input': ['text']},
        ],
        'refs': ['glm5/glm-4-flash', 'glm5/glm-4-air'],
    },
}
...
if models_dir.is_dir():
    for conf in sorted(models_dir.glob('*.conf')):
        ...
        provider = values.get('MODEL_OPENCLAW_PROVIDER')
...
for json_file in json_files:
    ...
    for provider, spec in provider_specs.items():
        if provider not in configured:
            if env_values.get(spec['env_var']) and any(ref in allowed_models for ref in spec['refs']):
                configured.add(provider)
            else:
                continue
        ...
        if providers.get(provider) != desired:
            providers[provider] = desired
            changed = True
        for ref in spec['refs']:
            if ref not in allowed_models:
                allowed_models[ref] = {}
                changed = True
    if changed:
        json_file.write_text(json.dumps(cfg, ensure_ascii=False, indent=2) + "\n", encoding='utf-8')
PY
    chown openclaw:openclaw "$OPENCLAW_HOME/.openclaw/openclaw.json" 2>/dev/null || true
    chown openclaw-svc:openclaw-svc "$OPENCLAW_SVC_HOME/.openclaw/openclaw.json" 2>/dev/null || true
    chmod 0600 "$OPENCLAW_HOME/.openclaw/openclaw.json" "$OPENCLAW_SVC_HOME/.openclaw/openclaw.json" 2>/dev/null || true
}
